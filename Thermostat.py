...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        self.updateLights()     ## Updates LED lights when changing states
        if(DEBUG):
    ...
    def on_exit_heat(self):
        redLight.off()          ## Turns off red light when exiting heat
    ##
    ...
    def on_exit_cool(self):
        blueLight.off()         ## Turns off blue light when exiting cool
   ##
    ## on_enter_off - Action performed when the state machine transitions
    ## into the 'off' state
    ##
    def on_enter_off(self):
        ## Turns off both LED lights when state is off
        redLight.off()
        blueLight.off()
        if(DEBUG):
    ...
    def processTempStateButton(self):
        ...
        self.cycle()            ## Cycles state machine between states
        self.updateLights()     ## Refreshes LED's after state change
    ...
    def processTempIncButton(self):
        ...
        self.setPoint += 1      ## Increments setpoint by 1 degree
        self.updateLights()     ## Updates LED's to reflect new setpoint
    ##
    ...
    def updateLights(self):
        ...
        ##  Red LED pulses if below setpoint in heat mode
        if self.current_state == self.heat:
            if temp < self.setPoint:
                redLight.pulse()
                ## Else redlight remains on if in heat
            else:
                redLight.on()

        ##  Green LED pulses if above setpoint in cool mode
        elif self.current_state == self.cool:
            if temp > self.setPoint:
                blueLight.pulse()
                ## Else blue light remains on if in cool
            else:
                blueLight.on()
    ...
    def setupSerialOutput(self):
        ## Constructs comma-delimited string for serial output
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## Formats LCD first line with date/time using formatted string
            lcd_line_1 = current_time.strftime("%m/%d %H:%M:%S").ljust(16) + "\n"

            ## Setup Display Line 2
            if(altCounter < 6):
                temp_f = floor(self.getFahrenheit())
                lcd_line_2 = f"Temp:{temp_f:>3}F".ljust(16)     ## Formats second line with temperature
                altCounter = altCounter + 1
            else:
                mode = self.current_state.id.capitalize()
                lcd_line_2 = f"{mode} SP:{self.setPoint:>3}F".ljust(16)     ## Displays mode and setpoint

                altCounter = altCounter + 1
                ## Updates lights every 10 cycles
                if(altCounter >= 11):
                    self.updateLights()
                    altCounter = 1

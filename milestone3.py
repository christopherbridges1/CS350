...
class CWMachine(StateMachine):
    ...
    ##
    def run(self):
        self._thread = Thread(target=self.transmit)
        self._thread.start()
    ...
    def transmit(self):
        while not self.endTransmission:
            # Force uppercase to match morseDict
            self.screen.updateScreen(f"Sending:\n{self.activeMessage}")
            wordList = self.activeMessage.upper().split()
            ...
            for word in wordList:
                ...
                for char in word:
                    ...
                    print(f"Char: {char}, Morse: {morse}")  # Debug output
                    ...
                    # Determines if dot or dash, double call to turn on and back off again
                    for x in morse:
                        if x == ".":
                            self.doDot()
                            self.doDot()
                        ...
                        # If another symbol in message, transition to appropriate state and increment morseCounter +1
                        if morseCounter < lenMorse:
                            self.doDDP()
                            self.doDDP()
                            morseCounter += 1

                # If another letter in message, transition to appropriate state and increment wordCounter +1
                if wordCounter < lenWord:
                    self.doLP()
                    self.doLP()
                    wordCounter += 1

            # If another word in message, transition to appropriate state and increment wordsCounter +1
